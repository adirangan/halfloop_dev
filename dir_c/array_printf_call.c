...
void bitstring_from_ulli(unsigned long long int *w_, char *str_, int k)
{
  /* extracts the first k bits of unsigned long long int *w_ and stores as output *str_ of zeros and ones */
  /* test with: b1=0;do{ bitstring_from_uchar(&b1,bstr_1,8); printf(" %% (%u --> %s)",(int)b1,bstr_1);} while (++b1!=0); printf("\n"); */
  unsigned long long int wc = 0;
  int i=0,j=0,l=0;
  *(str_+k) = '\0';
  while (i<k){
    j=i/64;l=63-(i%64); wc=w_[j];
    str_[i] = ((wc >> l) & 1) + '0';
    i++; /* while (i<k){ } */}
}
