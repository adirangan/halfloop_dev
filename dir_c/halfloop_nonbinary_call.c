...
void halfloop_nonbinary_f(int n_r,int n_c,float *A1_rc__,float *A1_cr__,double gamma,int **out_xdrop_p_,int *n_iteration_p,double **trace_p_)
{
  ...
  if (verbose){ printf(" %% [entering halfloop_nonbinary_f] n_r %d n_c %d gamma %0.2f\n",n_r,n_c,gamma);}
  ...
  for (nc=0;nc<n_c;nc++){
    ...
    dp_ps_immintrin_loadu_wrap(n_r,er_,A1_rc_,&(etA1n_[nc]));
    dp_ps_immintrin_loadu_wrap(n_r,A1_rc_,A1_rc_,&(etA2n_[nc]));
    QC_pre_[nc] = etA1n_[nc]*etA1n_[nc] - etA2n_[nc];
    /* for (nc=0;nc<n_c;nc++){ } */}
  for (nr=0;nr<n_r;nr++){
    A1_cr_ = A1_cr__ + (unsigned long long int)nr*(unsigned long long int)n_c;
    dp_ps_immintrin_loadu_wrap(n_c,etA1n_,A1_cr_,&(tmp_f_0));
    dp_ps_immintrin_loadu_wrap(n_c,A1_cr_,A1_cr_,&(tmp_f_1));
  ...
  for (niteration=0;niteration<n_iteration;niteration++){
    ...
    dp_ps_immintrin_loadu_wrap(n_r,er_,QR_pre_,&(tmp_f_0));
    trace_trn__[3+6*niteration] = tmp_f_0/n_r;
    dp_ps_immintrin_loadu_wrap(n_c,ec_,QC_pre_,&(tmp_f_1));
	...
	/* for (nc=0;nc<n_c_rmv;nc++){ } */}
      if (n_c_rtn>n_c/4){ dp_ps_immintrin_loadu_wrap(n_c,A1_cr_,etx1n_stretch_,&(wnxten_[nr]));}
      else{
	wnxten_[nr] = 0;
	for (nc=0;nc<n_c_rtn;nc++){
  ...
  free1(&ckeep_);
  /* %%%%%%%%%%%%%%%% */  
  if (verbose){ printf(" %% [finished halfloop_nonbinary_f] n_r %d n_c %d gamma %0.2f\n",n_r,n_c,gamma);}
}
...
void halfloop_nonbinary_d(int n_r,int n_c,double *B1_rc__,double *B1_cr__,double gamma,int **out_xdrop_p_,int *n_iteration_p,double **trace_p_)
{
  ...
  if (verbose){ printf(" %% [entering halfloop_nonbinary_d] n_r %d n_c %d gamma %0.2f\n",n_r,n_c,gamma);}
  ...
  for (nc=0;nc<n_c;nc++){
    ...
    dp_pd_immintrin_loadu_wrap(n_r,er_,B1_rc_,&(etB1n_[nc]));
    dp_pd_immintrin_loadu_wrap(n_r,B1_rc_,B1_rc_,&(etB2n_[nc]));
    QC_pre_[nc] = etB1n_[nc]*etB1n_[nc] - etB2n_[nc];
    /* for (nc=0;nc<n_c;nc++){ } */}
  for (nr=0;nr<n_r;nr++){
    B1_cr_ = B1_cr__ + (unsigned long long int)nr*(unsigned long long int)n_c;
    dp_pd_immintrin_loadu_wrap(n_c,etB1n_,B1_cr_,&(tmp_d_0));
    dp_pd_immintrin_loadu_wrap(n_c,B1_cr_,B1_cr_,&(tmp_d_1));
  ...
  for (niteration=0;niteration<n_iteration;niteration++){
    ...
    trace_trn__[1+6*niteration] = n_index_r_rem;
    trace_trn__[2+6*niteration] = n_index_c_rem;
    dp_pd_immintrin_loadu_wrap(n_r,er_,QR_pre_,&(tmp_d_0));
    trace_trn__[3+6*niteration] = tmp_d_0/n_r;
    dp_pd_immintrin_loadu_wrap(n_c,ec_,QC_pre_,&(tmp_d_1));
	...
	/* for (nc=0;nc<n_c_rmv;nc++){ } */}
      if (n_c_rtn>n_c/4){ dp_pd_immintrin_loadu_wrap(n_c,B1_cr_,etx1n_stretch_,&(wnxten_[nr]));}
      else{
	wnxten_[nr] = 0;
	for (nc=0;nc<n_c_rtn;nc++){
  ...
  free1(&ckeep_);
  /* %%%%%%%%%%%%%%%% */  
  if (verbose){ printf(" %% [finished halfloop_nonbinary_d] n_r %d n_c %d gamma %0.2f\n",n_r,n_c,gamma);}
}
...
void halfloop_nonbinary_test_error()
{
  ...
  if (verbose){ printf(" %% [entering halfloop_nonbinary_test_error]\n");}
  ...
  /* %%%%%%%% */
  GLOBAL_tic(0);  
  halfloop_nonbinary_f(n_r,n_c,A1_rc__,A1_cr__,gamma,&out_xdrop_trn__,&n_iteration,&trace_trn__);
  ...
  free1(&trace_trn__);
  GLOBAL_toc(0,1," halfloop_nonbinary_f: ");
  /* %%%%%%%% */
  GLOBAL_tic(0);  
  halfloop_nonbinary_d(n_r,n_c,B1_rc__,B1_cr__,gamma,&out_xdrop_trn__,&n_iteration,&trace_trn__);
  array_printf(trace_trn__,"double",6,minimum(8,n_iteration)," % trace_trn__: ");
  printf(" %% out_xdrop_trn_ans_ vs out_xdrop_trn__: relative error %0.16f (may not be small)\n",ifnormn(2*(n_r+n_c),out_xdrop_trn_ans_,out_xdrop_trn__));
  printf(" %% trace_trn_ans_ vs trace_trn__: relative error %0.16f (should be small)\n",dfnormn(n_iteration,trace_trn_ans_,trace_trn__));
  free1(&out_xdrop_trn__);
  free1(&trace_trn__);
  GLOBAL_toc(0,1," halfloop_nonbinary_d: ");
  /* %%%%%%%% */
  free1(&A1_rc__);
  free1(&A1_cr__);
  free1(&B1_rc__);
  free1(&B1_cr__);
  /* %%%%%%%% */
  if (verbose){ printf(" %% [finished halfloop_nonbinary_test_error]\n");}
}
...
void halfloop_nonbinary_test_speed()
{
  ...
  int nr=0,nc=0;
  float f=0;
  if (verbose){ printf(" %% [entering halfloop_nonbinary_test_speed]\n");}
  A1_rc__ = (float *) malloc1(n_r*n_c*sizeof(float));
  A1_cr__ = (float *) malloc1(n_r*n_c*sizeof(float));
  B1_rc__ = (double *) malloc1(n_r*n_c*sizeof(double));
  B1_cr__ = (double *) malloc1(n_r*n_c*sizeof(double));
  ...
  free1(&B1_cr__);
  /* %%%%%%%% */
  if (verbose){ printf(" %% [finished halfloop_nonbinary_test_speed]\n");}
}
...
void halfloop_nonbinary_rdrop_f_bkp(int n_r,int n_c,float *A1_rc__,float *A1_cr__,double gamma,int **out_xdrop_p_,int *n_iteration_p,double **trace_p_)
{
  ...
  int nx=0,niteration=0;
  float tmp_f_0=0,tmp_f_1=0;
  if (verbose){ printf(" %% [entering halfloop_nonbinary_rdrop_f] n_r %d n_c %d gamma %0.2f\n",n_r,n_c,gamma);}
  if (verbose>1){ farray_printf_margin(A1_rc__,n_r,n_c," % A1_rc__: ");printf("\n");}
  if (verbose>1){ farray_printf_margin(A1_cr__,n_c,n_r," % A1_cr__: ");printf("\n");}
  /* %%%%%%%%%%%%%%%% */  
  get_xdrop_logscale_array(n_r,maximum(n_r,n_c),gamma,&n_iteration,&rdrop_,&cdrop_,&rkeep_,&ckeep_);
  ...
  free1(&ckeep_);
  /* %%%%%%%%%%%%%%%% */  
  if (verbose){ printf(" %% [finished halfloop_nonbinary_rdrop_f] n_r %d n_c %d gamma %0.2f\n",n_r,n_c,gamma);}
}
...
void halfloop_nonbinary_rdrop_f(int n_r,int n_c,float *A1_rc__,float *A1_cr__,double gamma,int **out_xdrop_p_,int *n_iteration_p,double **trace_p_)
{
  ...
  for (niteration=0;niteration<n_iteration;niteration++){
    ...
    for (nr=0;nr<n_r_rtn;nr++){
      A1_cr_ = A1_cr__ + (unsigned long long int)(r_rtn_[nr]) * (unsigned long long int)n_c;
      dp_ps_immintrin_loadu_wrap(n_c,A1_cr_,etx1n_,&(wnxten_[nr]));
      /* for (nr=0;nr<n_r_rtn;nr++){ } */}
    memset(QR_pos_,0,n_r*sizeof(float));
    for (nr=0;nr<n_r_rtn;nr++){
...
void halfloop_nonbinary_rdrop_d_bkp(int n_r,int n_c,double *A1_rc__,double *A1_cr__,double gamma,int **out_xdrop_p_,int *n_iteration_p,double **trace_p_)
{
  ...
  int nx=0,niteration=0;
  double tmp_d_0=0,tmp_d_1=0;
  if (verbose){ printf(" %% [entering halfloop_nonbinary_rdrop_d] n_r %d n_c %d gamma %0.2f\n",n_r,n_c,gamma);}
  ...
  for (nc=0;nc<n_c;nc++){
    A1_rc_ = A1_rc__ + (unsigned long long int)nc*(unsigned long long int)n_r;
    dp_pd_immintrin_loadu_wrap(n_r,er_,A1_rc_,&(etA1n_[nc]));
    dp_pd_immintrin_loadu_wrap(n_r,A1_rc_,A1_rc_,&(etA2n_[nc]));
    QC_pre_[nc] = etA1n_[nc]*etA1n_[nc] - etA2n_[nc];
    /* for (nc=0;nc<n_c;nc++){ } */}
  for (nr=0;nr<n_r;nr++){
    A1_cr_ = A1_cr__ + (unsigned long long int)nr*(unsigned long long int)n_c;
    dp_pd_immintrin_loadu_wrap(n_c,etA1n_,A1_cr_,&(tmp_d_0));
    dp_pd_immintrin_loadu_wrap(n_c,A1_cr_,A1_cr_,&(tmp_d_1));
    QR_pre_[nr] = tmp_d_0 - tmp_d_1;
    /* for (nr=0;nr<n_r;nr++){ } */}
  ...
  for (niteration=0;niteration<n_iteration;niteration++){
	...
	y2ny2n_[nr] += tmp_d_0*tmp_d_0;
	/* for (nc=0;nc<n_c_rmv;nc++){ } */}
      if (n_c_rtn>n_c/4){ dp_pd_immintrin_loadu_wrap(n_c,A1_cr_,etx1n_stretch_,&(wnxten_[nr]));}
      else{
	wnxten_[nr] = 0;
	for (nc=0;nc<n_c_rtn;nc++){
  ...
  free1(&ckeep_);
  /* %%%%%%%%%%%%%%%% */  
  if (verbose){ printf(" %% [finished halfloop_nonbinary_rdrop_d] n_r %d n_c %d gamma %0.2f\n",n_r,n_c,gamma);}
}
...
void halfloop_nonbinary_rdrop_d(int n_r,int n_c,double *A1_rc__,double *A1_cr__,double gamma,int **out_xdrop_p_,int *n_iteration_p,double **trace_p_)
{
  ...
  for (niteration=0;niteration<n_iteration;niteration++){
    ...
    for (nr=0;nr<n_r_rtn;nr++){
      A1_cr_ = A1_cr__ + (unsigned long long int)(r_rtn_[nr]) * (unsigned long long int)n_c;
      dp_pd_immintrin_loadu_wrap(n_c,A1_cr_,etx1n_,&(wnxten_[nr]));
      /* for (nr=0;nr<n_r_rtn;nr++){ } */}
    memset(QR_pos_,0,n_r*sizeof(double));
    for (nr=0;nr<n_r_rtn;nr++){
      QR_pos_[r_rtn_[nr]] = QR_pre_[r_rtn_[nr]] - wnxten_[nr];
...
void halfloop_nonbinary_rdrop_test_error()
{
  ...
  if (verbose){ printf(" %% [entering halfloop_nonbinary_rdrop_test_error]\n");}
  A1_rc__ = (float *) malloc1(n_r*n_c*sizeof(float));
  A1_cr__ = (float *) malloc1(n_r*n_c*sizeof(float));
  B1_rc__ = (double *) malloc1(n_r*n_c*sizeof(double));
  B1_cr__ = (double *) malloc1(n_r*n_c*sizeof(double));
  ...
  /* %%%%%%%% */
  GLOBAL_tic(0);  
  halfloop_nonbinary_rdrop_f_bkp(n_r,n_c,A1_rc__,A1_cr__,gamma,&out_xdrop_trn__,&n_iteration,&trace_trn__);
  array_printf(trace_trn__,"double",6,minimum(8,n_iteration)," % trace_trn__: ");
  printf(" %% out_xdrop_trn_ans_ vs out_xdrop_trn__: relative error %0.16f (may not be small)\n",ifnormn(2*n_r,out_xdrop_trn_ans_,out_xdrop_trn__));
  printf(" %% trace_trn_ans_ vs trace_trn__: relative error %0.16f (should be small)\n",dfnormn(n_iteration,trace_trn_ans_,trace_trn__));
  free1(&out_xdrop_trn__);
  free1(&trace_trn__);
  GLOBAL_toc(0,1," halfloop_nonbinary_rdrop_f_bkp: ");
  /* %%%%%%%% */
  GLOBAL_tic(0);  
  halfloop_nonbinary_rdrop_f(n_r,n_c,A1_rc__,A1_cr__,gamma,&out_xdrop_trn__,&n_iteration,&trace_trn__);
  array_printf(trace_trn__,"double",6,minimum(8,n_iteration)," % trace_trn__: ");
  printf(" %% out_xdrop_trn_ans_ vs out_xdrop_trn__: relative error %0.16f (may not be small)\n",ifnormn(2*n_r,out_xdrop_trn_ans_,out_xdrop_trn__));
  ...
  free1(&out_xdrop_trn__);
  free1(&trace_trn__);
  GLOBAL_toc(0,1," halfloop_nonbinary_rdrop_f: ");
  /* %%%%%%%% */
  GLOBAL_tic(0);  
  halfloop_nonbinary_rdrop_d_bkp(n_r,n_c,B1_rc__,B1_cr__,gamma,&out_xdrop_trn__,&n_iteration,&trace_trn__);
  array_printf(trace_trn__,"double",6,minimum(8,n_iteration)," % trace_trn__: ");
  printf(" %% out_xdrop_trn_ans_ vs out_xdrop_trn__: relative error %0.16f (may not be small)\n",ifnormn(2*n_r,out_xdrop_trn_ans_,out_xdrop_trn__));
  printf(" %% trace_trn_ans_ vs trace_trn__: relative error %0.16f (should be small)\n",dfnormn(n_iteration,trace_trn_ans_,trace_trn__));
  free1(&out_xdrop_trn__);
  free1(&trace_trn__);
  GLOBAL_toc(0,1," halfloop_nonbinary_rdrop_d_bkp: ");
  /* %%%%%%%% */
  GLOBAL_tic(0);  
  halfloop_nonbinary_rdrop_d(n_r,n_c,B1_rc__,B1_cr__,gamma,&out_xdrop_trn__,&n_iteration,&trace_trn__);
  array_printf(trace_trn__,"double",6,minimum(8,n_iteration)," % trace_trn__: ");
  printf(" %% out_xdrop_trn_ans_ vs out_xdrop_trn__: relative error %0.16f (may not be small)\n",ifnormn(2*n_r,out_xdrop_trn_ans_,out_xdrop_trn__));
  printf(" %% trace0_trn_ans_ vs trace_trn__: relative error %0.16f (should be small)\n",dfnormn(n_iteration,trace0_trn_ans_,trace_trn__));
  free1(&out_xdrop_trn__);
  free1(&trace_trn__);
  GLOBAL_toc(0,1," halfloop_nonbinary_rdrop_d: ");
  /* %%%%%%%% */
  free1(&A1_rc__);
  free1(&A1_cr__);
  free1(&B1_rc__);
  free1(&B1_cr__);
  /* %%%%%%%% */
  if (verbose){ printf(" %% [finished halfloop_nonbinary_rdrop_test_error]\n");}
}
...
void halfloop_nonbinary_rdrop_test_speed()
{
  ...
  int nr=0,nc=0;
  float f=0;
  if (verbose){ printf(" %% [entering halfloop_nonbinary_rdrop_test_speed]\n");}
  A1_rc__ = (float *) malloc1(n_r*n_c*sizeof(float));
  A1_cr__ = (float *) malloc1(n_r*n_c*sizeof(float));
  B1_rc__ = (double *) malloc1(n_r*n_c*sizeof(double));
  B1_cr__ = (double *) malloc1(n_r*n_c*sizeof(double));
  ...
  free1(&B1_cr__);
  /* %%%%%%%% */
  if (verbose){ printf(" %% [finished halfloop_nonbinary_rdrop_test_speed]\n");}
}
