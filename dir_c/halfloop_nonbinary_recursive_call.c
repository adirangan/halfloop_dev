...
)
{
  ...
  if (n_r_index> n_m_index){
    ...
    for (niteration=0;niteration<n_iteration;niteration++){
      ...
      dp_ps_mult_immintrin_loadu(n_m_index,n_m_index,Q_rc__,n_c_index,QE_0_rc__,&QE_1_rc__);
...
)
{
  ...
  FILE *fp=NULL;
  if (verbose>0){ printf(" %% [entering halfloop_nonbinary_f_recursive]\n");}
  /* %%%%%%%%%%%%%%%% */
  ...
  *binary_label_p_ = (unsigned  long long int *) malloc1((unsigned long long int)n_r_index*sizeof(unsigned long long int));
  ...
  output_label__ = NULL; malloc1_char__(n_r_index,output_label_p_); output_label__ = *output_label_p_;
  if (output_label__!=NULL){ for (nr_index=0;nr_index<n_r_index;nr_index++){ sprintf(output_label__[nr_index],"0");}}
  nlpbra_label__ = NULL; malloc1_char__(n_r_index,nlpbra_label_p_); nlpbra_label__ = *nlpbra_label_p_;
  if (nlpbra_label__!=NULL){ for (nr_index=0;nr_index<n_r_index;nr_index++){ sprintf(nlpbra_label__[nr_index],"");}}
  nlpnex_label__ = NULL; malloc1_char__(n_r_index,nlpnex_label_p_); nlpnex_label__ = *nlpnex_label_p_;
  ...
  if (flag_r0drop_vs_rcdrop==flag_r0drop){
    if (verbose>1){ printf(" %% using all coluns, skipping variable-selection.\n");}
  ...
  if (flag_split){
    ...
    for (nr_rtn_index_F=0;nr_rtn_index_F<n_r_rtn_index_F;nr_rtn_index_F++){
      ...
      if (binary_label_!=NULL){ if (binary_label_A_[nr_rtn_index_F]>(unsigned long long int)(4.295e9)){ printf(" %% Warning, too many clusters, increase label length\n");} binary_label_[nr_index] += (unsigned long long int)0+(unsigned long long int)2*binary_label_A_[nr_rtn_index_F];}
    ...
    for (nr_rmv_index_F=0;nr_rmv_index_F<n_r_rmv_index_F;nr_rmv_index_F++){
      ...
      if (binary_label_!=NULL){ if (binary_label_B_[nr_rmv_index_F]>(unsigned long long int)(4.295e9)){ printf(" %% Warning, too many clusters, increase label length\n");} binary_label_[nr_index] += (unsigned long long int)1+(unsigned long long int)2*binary_label_B_[nr_rmv_index_F];}
  ...
  free1(&MDA_dim_);
  /* %%%%%%%%%%%%%%%% */
  if (verbose>0){ printf(" %% [finished halfloop_nonbinary_f_recursive]\n");}
}
...
)
{
  ...
  char fname_nlpnex_label[PNAMESIZE];
  FILE *fp=NULL;
  if (verbose>0){ printf(" %% [entering halfloop_nonbinary_f_recursive_omp]\n");}
  /* %%%%%%%%%%%%%%%% */
  n_str_path = pathconf(".",_PC_PATH_MAX);
  MDA_dim_ = (int *) malloc1(2*sizeof(int)); //%<-- should only ever need 2. ;
  nlp_p_split_ = (double *) malloc1(9*sizeof(double)); //%<-- gumb_emp, gumb_opt, F, E, p_branch, p_next, n_r_rtn_index_F, n_r_rmv_index_F, flag_split. ;
  ...
  *binary_label_p_ = (unsigned  long long int *) malloc1((unsigned long long int)n_r_index*sizeof(unsigned long long int));
  ...
  free1(&MDA_dim_);
  /* %%%%%%%%%%%%%%%% */
  if (verbose>0){ printf(" %% [finished halfloop_nonbinary_f_recursive_omp]\n");}
}
...
)
{
 ...
 ,&nlpnex_label__
  ...
  /* if (flag_omp_use==1){ } */}
  for (nr=0;nr<n_r;nr++){ binary_label_out_[nr] = binary_label_[nr];}  
  free1(&binary_label_);
